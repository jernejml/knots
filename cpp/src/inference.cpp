...
LetterboxParams Letterbox(const cv::Mat& src, cv::Mat& dst, int target_size, int pad_value) {
    const int h0 = src.rows;
    const int w0 = src.cols;
    const float scale =
        std::min(static_cast<float>(target_size) / h0, static_cast<float>(target_size) / w0);
    ...
    dst =
        cv::Mat(target_size, target_size, src.type(), cv::Scalar(pad_value, pad_value, pad_value));
...
cv::Point2f InverseLetterboxPoint(const cv::Point2f& pt, const LetterboxParams& lb) {
    return cv::Point2f((pt.x - lb.pad_x) / lb.scale, (pt.y - lb.pad_y) / lb.scale);
}

// Same, but for a bbox.
cv::Rect2f InverseLetterboxBox(float x1, float y1, float x2, float y2, const LetterboxParams& lb) {
...
std::vector<Detection> InferFrame(Ort::Session& session, const cv::Mat& bgr_image,
    ...
    cv::Mat blob;
    cv::dnn::blobFromImage(letterboxed, blob, 1.0 / 255.0, cv::Size(), cv::Scalar(),
                           /*swapRB=*/true, /*crop=*/false, CV_32F);

    std::vector<int64_t> input_shape = {1, 3, kNetSize, kNetSize};
    Ort::MemoryInfo mem_info = Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
    Ort::Value input_tensor =
        Ort::Value::CreateTensor<float>(mem_info, reinterpret_cast<float*>(blob.data), blob.total(),
    ...
    auto outputs = session.Run(Ort::RunOptions{nullptr}, in_names, &input_tensor, 1, out_names, 2);
    ...
    if (out1_shape.size() != 4 || out1_shape[1] != kNumMaskCoefs || out1_shape[2] != kProtoSize ||
    ...
    cv::Mat protos(kNumMaskCoefs, kProtoSize * kProtoSize, CV_32F, const_cast<float*>(proto_data));
    ...
    for (int i = 0; i < n_dets; ++i) {
        ...
        cv::Mat mask_net;
        cv::resize(mask_bin, mask_net, cv::Size(kNetSize, kNetSize), 0, 0, cv::INTER_LINEAR);
        ...
        for (const auto& p : simplified) {
            ...
            poly_src.emplace_back(std::clamp(static_cast<int>(std::round(q.x)), 0, src_w - 1),
